 ...
 * @param {object} pool - Pool PostgreSQL
 * @param {object} params - { slug, title, message, parse_mode, trigger, scheduled_at, filters, created_by }
 * @returns {Promise<object>}
 */
async function createShot(pool, params) {
  ...
  const query = `
    INSERT INTO public.shots (
      bot_slug, slug, title, content, message, parse_mode, trigger, scheduled_at, 
      filters, status, created_by, created_at, updated_at
    )
    VALUES ($1, $1, $2, $3, $3, $4, $5, $6, $7, $8, $9, now(), now())
    RETURNING id, bot_slug, slug, title, content, message, parse_mode, trigger, scheduled_at, 
  ...
  const result = await pool.query(query, [
    slug,
    title,
    messageJson,  // Usado para content e message
    finalParseMode,
    trigger,
    scheduled_at || null,
