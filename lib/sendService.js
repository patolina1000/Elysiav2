...
const { startCleanup } = require('./rateLimiterCleanup');
...
/**
 * Envia mídia (photo/video/document) com estratégia cache-first
...
async function sendMediaMessage(pool, params) {
  ...
  const {
    ...
    raw = false  // Se true, não escapa o caption
  } = params;
  
  // Validações
  if (!slug || !chat_id || !media_sha256 || !media_kind) {
    return {
      ok: false,
      error: 'MISSING_REQUIRED_PARAMS',
      lat_ms: Date.now() - startTime
    };
  }
  ...
  try {
    ...
    if (!token) {
      console.warn('[SEND][MEDIA][NO_TOKEN]', { slug, purpose });
    ...
    // 3. Tentar cache PRIMEIRO (cache-first strategy)
    const cacheStart = Date.now();
    const cached = await getCachedFileId(pool, slug, media_sha256, media_kind);
    const cacheLookupMs = Date.now() - cacheStart;
    
    observe('media_cache_lookup_ms', cacheLookupMs, { bot: slug, kind: media_kind });
    
    if (cached && cached.file_id) {
      // CACHE HIT - envio rápido com file_id
      console.info('[SEND][MEDIA][CACHE_HIT]', {
        slug,
        sha256: media_sha256,
        kind: media_kind,
        file_id: cached.file_id.slice(0, 20) + '...'
      });
      ...
      switch (media_kind) {
        ...
        case 'document':
        case 'audio':
          result = await telegramClient.sendDocument(token, chat_id, cached.file_id, {
            caption: finalCaption,
            parse_mode
          });
          break;
      ...
      // Métricas
      observe('send_media_attempt_ms', totalLat, { bot: slug, kind: media_kind, cache_hit: true });
      if (result.http_latency_ms) {
        observe('telegram_http_ms', result.http_latency_ms, { bot: slug, method: `send_${media_kind}` });
      }
      
      if (result.ok) {
        recordMetric(slug, purpose, 'ok', 1);
        
        console.info('[SEND][MEDIA][OK]', {
          request_id,
          slug,
          chat_id,
          kind: media_kind,
    ...
    // CACHE MISS - não enviar no hot-path, retornar MEDIA_NOT_READY
    console.warn('[SEND][MEDIA][CACHE_MISS]', {
      slug,
      sha256: media_sha256,
      kind: media_kind,
      r2_key: media_r2_key
    ...
    observe('media_cache_miss_total', 1, { bot: slug, kind: media_kind });
    recordMetric(slug, purpose, 'err', 1);
    
    // Criar entrada de cache (status: warming)
    await createCacheEntry(pool, slug, media_sha256, media_kind).catch(err => {
      console.error('[SEND][MEDIA][CREATE_CACHE][ERR]', { error: err.message });
    });
    
    // Enfileirar para aquecimento (se r2_key fornecido)
    if (media_r2_key) {
      enqueuePrewarm({
        bot_slug: slug,
        sha256: media_sha256,
    ...
    return {
      ...
      description: 'Media is being warmed up. Please retry in a few seconds.',
      warming: true,
      lat_ms: Date.now() - startTime
    };
    ...
    console.error('[SEND][MEDIA][EXCEPTION]', {
      request_id,
      slug,
      chat_id,
      kind: media_kind,
...
const queueProcessor = setInterval(processQueue, 100);
// Permitir que processo termine mesmo com timer ativo (previne open handle em testes)
...
// Iniciar cleanup automático de rate limiters (previne memory leak)
startCleanup(rateLimiters);
console.info('[RATE_LIMITER][CLEANUP] Iniciado (intervalo: 5min, idle: 10min)');
