...
const metrics = {
  ...
  // Multi-Media (new)
  multi_media_send: new Map(), // bot:media_count -> [latencies]
  media_send_attempt: new Map(), // bot:kind -> [latencies]
  media_send_error: new Map(), // bot:kind:error -> count
...
function observe(name, value, labels = {}) {
  ...
  // Multi-Media
  else if (name === 'multi_media_send_ms') {
    const mediaCount = labels.media_count || 1;
    const key = `${bot}:${mediaCount}`;
    if (!metrics.multi_media_send.has(key)) metrics.multi_media_send.set(key, []);
    pushSample(metrics.multi_media_send.get(key), value);
  }
  else if (name === 'media_send_attempt_ms') {
    const kind = labels.kind || 'unknown';
    const key = `${bot}:${kind}`;
    if (!metrics.media_send_attempt.has(key)) metrics.media_send_attempt.set(key, []);
    pushSample(metrics.media_send_attempt.get(key), value);
  }
  else if (name === 'media_send_error') {
    const kind = labels.kind || 'unknown';
    const error = labels.error || 'unknown';
    const key = `${bot}:${kind}:${error}`;
    metrics.media_send_error.set(key, (metrics.media_send_error.get(key) || 0) + value);
  }
...
function getMetrics() {
  ...
  // Multi-Media
  for (const [key, values] of metrics.multi_media_send.entries()) {
    result.multi_media.send_ms[key] = calculatePercentiles(values);
  }
  for (const [key, values] of metrics.media_send_attempt.entries()) {
    result.multi_media.attempt_ms[key] = calculatePercentiles(values);
  }
  for (const [key, count] of metrics.media_send_error.entries()) {
    result.multi_media.error_total[key] = count;
  }
