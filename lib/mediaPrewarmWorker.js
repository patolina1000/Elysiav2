...
function calculatePriority(params) {
  const { kind, bytes, enqueued_at, priority: explicitPriority } = params;
  let priority = 100; // Base
  
  // Usar prioridade explícita se fornecida (para multi-media)
  if (explicitPriority) {
    ...
    // Priorizar por tipo (audio > video > photo para multi-media)
    if (kind === 'audio') {
      priority += 30; // Áudio tem prioridade máxima
    } else if (kind === 'photo') {
...
function enqueuePrewarm(params) {
  const { bot_slug, sha256, kind, r2_key, bytes, priority } = params;
  ...
  const job = {
    ...
    r2_key,
    bytes,
    priority, // Passar prioridade explícita se fornecida
